...
class fesom_mesh(object):
    ...
    mesh : object
        fesom_mesh object
"""
    def __init__(self, path, abg = [50, 15, -90], get3d = True, cavity = True):
        self.path=os.path.abspath(path)
        print('Find bottom layer indices and save as bind')
        ...
        self.cflag2dfile=os.path.join(self.path,'cavity_flag_nod2d.out')
    ...
    def read2d(self):
        ...
        file_content = pd.read_csv(self.cflag2dfile, skiprows=0, names=['cflag'] )
        self.cflag     = file_content.cflag.values
    ...
    def read3d(self,cavity):
        ...
        ### Correct for cavity by OR ###########
        if cavity:
            print('correct for cavity')
        ...
        ### OR: get bottom index (for with and without cavity; usefull for plotting bottom layer quantities ######
        print('Find bottom layer indices and save as bind')
        self.bind=np.ones(self.n2d,dtype=int)*-999
        for i,prof in enumerate(self.n32):
            ii = np.where(prof!=-999)
            self.bind[i]=prof[ii[0][-1]]-1
        ######################################################################################################

        return self

    def read3d_sigma(self):
        '''
        Reads 3d part of the mesh. as sigma
        '''
        self.n3d=int(open(self.nod3dfile).readline().rstrip())
        df = pd.read_csv(self.nod3dfile, delim_whitespace=True, skiprows=1, \
                                    names=['node_number','x','y','z','flag'])
        zcoord = -df.z.values
        self.zcoord = zcoord
        self.zlevs = np.linspace(0,max(zcoord),100)
        self.ind3d  =df.flag.values
        ...
        self.sigma_lev = np.arange(self.n32.shape[1])+1
...
def interp_sigma2z(mesh_s,data_s,mesh_z):
    data_interp = np.empty(mesh_z.n3d)
    for i in np.arange(mesh_z.n2d):
        z0 = mesh_s.zcoord[mesh_s.n32[i]-1]
        data0 = data_s[mesh_s.n32[i]-1]
        tn = mesh_z.n32[i]>=0
        prof_intp = np.interp(mesh_z.zlevs[tn],z0,data0)
        data_interp[mesh_z.n32[i][tn]-1]=prof_intp
    return data_interp
...
def get_data_sigma(data, mesh, sigma = 0, verbose=True):
    '''
    Show data from the model level that is closest to the
    desired depth. 
    ...
    '''
    elem2=mesh.elem[mesh.no_cyclic_elem,:]
    level_data = data[mesh.n32[:,sigma]-1]
    #The data2[elem2] creates 3d array where every raw is three
    #values of the parameter on the verticies of the triangle.
    d=level_data[elem2].mean(axis=1)
    #k = [i for (i, val) in enumerate(d) if not np.isnan(val)]
